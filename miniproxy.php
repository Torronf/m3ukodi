...
Update for PHP V 8.0/By M3uKodi Telegram Group. 10/06/2023
*/
...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>miniProxy</title></head><body><h1>Bienvenidos al miniProxy!</h1>miniProxy se puede ejecutar directamente de la siguiente manera: <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "</a><br /><br />O simplemente puede ingresar una URL a continuación:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Proxy Esto!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
